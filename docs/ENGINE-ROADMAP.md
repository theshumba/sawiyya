...
- **Everyday signs = fake-feeling**: no dataset → `CameraTrainer` falls back to **teach mode** (you record your own hand, then it confirms you match _yourself_).
...
- No public **QSL isolated-sign** dataset exists → real everyday-sign grading/videos need recording a native signer. The engine work makes the _alphabet_ excellent and gives us the _capability_ for words; the _data_ for QSL words is a real-world step.
