...
function forwardWith(m: Model, x: number[]) {
  ...
  for (let j = 0; j < H; j++) {
    let s = m.b1[j];
    for (let i = 0; i < D; i++) s += x[i] * m.W1[i][j];
    h[j] = s;
  }
  ...
  for (let k = 0; k < K; k++) {
    let s = m.b2[k];
    for (let j = 0; j < H; j++) s += a[j] * m.W2[j][k];
    o[k] = s;
  }
  const mx = Math.max(...o);
  const e = o.map((z) => Math.exp(z - mx));
...
function accuracy(m: Model, rows: Row[]) {
  ...
  for (const r of rows) {
    ...
    let am = 0;
    for (let k = 1; k < K; k++) if (p[k] > p[am]) am = k;
    if (am === r.y) ok++;
  }
...
function trainModel(train: Row[], label: string): Model {
  ...
  const m: Model = {
    W1: mat(D, H, Math.sqrt(2 / D)),
    b1: new Array(H).fill(0),
    W2: mat(H, K, Math.sqrt(2 / H)),
    b2: new Array(K).fill(0),
  };
  ...
  for (let ep = 0; ep < EPOCHS; ep++) {
    ...
    for (let b = 0; b < rows.length; b += BATCH) {
      ...
      const gW1 = mat(D, H, 0),
        gb1 = new Array(H).fill(0),
        gW2 = mat(H, K, 0),
        gb2 = new Array(K).fill(0);
      for (const r of batch) {
        const { a, h, p } = forwardWith(m, r.x);
        const dO = p.slice();
        dO[r.y] -= 1; // softmax+CE grad
        for (let k = 0; k < K; k++) {
          gb2[k] += dO[k];
          for (let j = 0; j < H; j++) gW2[j][k] += a[j] * dO[k];
        }
        const dA = new Array(H).fill(0);
        for (let j = 0; j < H; j++) {
          let s = 0;
          for (let k = 0; k < K; k++) s += dO[k] * m.W2[j][k];
          dA[j] = h[j] > 0 ? s : 0;
        }
        for (let j = 0; j < H; j++) {
          gb1[j] += dA[j];
          for (let i = 0; i < D; i++) gW1[i][j] += r.x[i] * dA[j];
        }
      }
      const n = batch.length;
      for (let j = 0; j < H; j++) {
        m.b1[j] -= (lr * gb1[j]) / n;
        for (let i = 0; i < D; i++) m.W1[i][j] -= (lr * gW1[i][j]) / n;
      }
      for (let k = 0; k < K; k++) {
        m.b2[k] -= (lr * gb2[k]) / n;
        for (let j = 0; j < H; j++) m.W2[j][k] -= (lr * gW2[j][k]) / n;
      }
    }
...
function calibrate(m: Model, test: Row[]) {
  ...
  console.log(
    `CALIBRATE  tau=${tau}  TA=${(taAtTau * 100).toFixed(1)}%  FA=${(faAtTau * 100).toFixed(2)}%  (FA budget ${FA_BUDGET * 100}%)`,
  );
...
if (corpus) {
  ...
  console.log(
    `classes=${K} dim=${D}  zenodo train/test=${zen.train.length}/${zen.test.length}  arsl21l train/test=${ars.train.length}/${ars.test.length}`,
  );
  ...
  console.log(
    `BASELINE (zenodo-only)  zenodo held-out=${(baseWithin * 100).toFixed(1)}%  ArSL21L held-out (CROSS)=${(baseCross * 100).toFixed(1)}%`,
  );
  ...
  console.log(
    `FINAL (blended)  zenodo held-out=${(accZen * 100).toFixed(1)}%  ArSL21L held-out=${(accArs * 100).toFixed(1)}%  overall=${(accBlend * 100).toFixed(1)}%`,
  );
  ...
  const trainAcc = accuracy(m, train),
    testAcc = accuracy(m, test);
